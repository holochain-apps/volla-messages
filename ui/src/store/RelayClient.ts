...
export class RelayClient {
  ...
  async getRelayProvisionedCellInfo(): Promise<ProvisionedCell> {
    ...
    const cellInfo = appInfo.cell_info[ROLE_NAME].find(
      (c) => CellType.Provisioned in c
    );
    if (!cellInfo)
      throw new Error("Provisioned relay cell not found in appInfo");
  ...
  async setMyProfileForConversation(cell_id: CellId): Promise<Record> {
    const record = await this.getAgentProfile(
      this.provisionedRelayCellId,
  ...
  public async generateMembraneProof(
    ...
    if (conversation === undefined)
      throw new Error("Conversation with cell_id not found");
