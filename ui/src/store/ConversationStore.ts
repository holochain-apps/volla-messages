...
export function createConversationStore(
  ...
  const invitation = persisted<{
    [cellIdB64: CellIdB64]: Invitation | undefined;
  ...
  async function invite(
    key: CellIdB64,
    agentPubKeyB64s: AgentPubKeyB64[]
  ): Promise<void> {
    invited.update((c) => ({
      ...c,
  ...
  async function sendMessage(
    ...
    const agentPubKeys = Object.keys(get(mergedProfileContact)).map((a) =>
    ...
    if (message === undefined)
      throw new Error("Failed to decode Message entry from record");
  ...
  async function loadMessagesInBucket(key1: CellIdB64, bucket: number) {
    ...
    const missingActionHashs = difference(
      actionHashB64s,
      storedActionHashB64s
    ).map((h) => decodeHashFromBase64(h));
  ...
  async function handleMessageSignalReceived(
    ...
    const messageExtended = await _makeMessageExtended(
    ...
    const fromProfile =
      get(mergedProfileContact)[encodeHashToBase64(signal.from)];
  ...
  async function _triggerMessageNotification(
    ...
    const header = fromProfile
      ? `Message From ${fromProfile.profile.nickname}`
      : `New Message`;
  ...
  async function _makeMessageExtended(
    ...
    const messageFileExtendeds = await Promise.all(
      messageRecord.message.images.map(async (i) =>
        _loadMessageFile(fileStorageClient, i)
      )
    );
...
export function deriveCellConversationStore(
  ...
  return {
    ...
    loadMessagesInBucket: (bucket: number) =>
      conversationStore.loadMessagesInBucket(key, bucket),
...
export function deriveCellConversationMessagesListStore(
  ...
  const data = derived(cellConversationStore, ($cellConversationStore) => {
    ...
    return sortBy(Object.entries($cellConversationStore.messages), [
      (c) => c[1].timestamp,
    ]);
  });
...
export const deriveConversationListStore = (
  conversationStore: ConversationStore
) =>
