...
export function createConversationStore(
  ...
  const localData = persisted<LocalConversationData>(
    `CONVERSATION.${dnaHashB64}.LOCAL_DATA`,
    {
      archived: false,
      invitedContactKeys: [],
      unread: false,
  ...
  const isOpen = derived(
    page,
    ($page) =>
      $page.route.id === "/conversations/[id]" && $page.params.id === dnaHashB64
  );
    ...
    messages.forEach((message) => {
      ...
      if (
        ...
        message.timestamp.toDateString() !==
          newLastMessage.timestamp.toDateString()
      ) {
      ...
      if (
        ...
        message.timestamp.getTime() - newLastMessage.timestamp.getTime() <
          1000 * 60 * 5
      ) {
  ...
  async function loadMessageSetFromCurrentBucket(): Promise<
    [number, ActionHashB64[]]
  > {
  ...
  // the actual messages in that bucket as well as any earlier buckets necessary
  // such that at least TARGET_MESSAGES_COUNT messages.
  async function _loadMessageSetFrom(
    bucket: number
  ): Promise<[number, ActionHashB64[]]> {
          ...
          try {
            ...
            if (message) {
              ...
              if (!newMessages[message.hash]) {
                const matchesPending = Object.values(
                  get(conversation).messages
                ).find(
                  (m) =>
                    m.status === "pending" &&
  ...
  async function sendMessage(
    ...
    const newMessageEntry = await client.sendMessage(
      ...
      bucket,
      imageStructs,
      Object.keys(get(conversation).agentProfiles).map((k) =>
        decodeHashFromBase64(k)
      )
    );
  ...
  async function loadImagesForMessage(message: Message) {
    ...
    const images = await Promise.all(
      message.images.map((image) => _loadImage(image))
    );
    conversation.update((c) => {
  ...
  function getInvitedUnjoinedContacts(): ContactExtended[] {
    ...
    return get(localData)
      ...
      .map((contactKey) => {
        const contactProfile = relayStore.contacts.find(
  ...
  function _getMembers(includeInvited: boolean): ProfileExtended[] {
    ...
    const keys = uniq(
      Object.keys(joinedAgents).concat(
        includeInvited ? get(localData).invitedContactKeys : []
      )
    );
