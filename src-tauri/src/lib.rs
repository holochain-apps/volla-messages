...
use tauri::{WebviewUrl, WebviewWindowBuilder, WebviewBuilder, WindowBuilder};
    ...
    #[cfg(feature = "holochain_service")]
    {
        builder
            ...
            .setup(|app| {
                ...
                let main_webview = WebviewWindowBuilder::new(&handle, "main", WebviewUrl::App("".into())) 
                    ...
                    .initialization_script(format!(r#"
                        window.__HAPP_BUNDLE_BYTES__ = {:?};
                    "#, HAPP_BUNDLE_BYTES).as_str())
                    .initialization_script(format!(
                        r#"
                            if (window.location.origin === 'http://tauri.localhost') {{
                                setTimeout(async () => {{
                                    ...
                                    if(!installed) {{
                                        await window.__TAURI_INTERNALS__.invoke('plugin:holochain-service-consumer|install_app', {{ appId: "{}", appBundleBytes: window.__HAPP_BUNDLE_BYTES__, membraneProofs: {{}}, networkSeed: "{}" }});
                                    }}
                                    window.__HAPP_BUNDLE_BYTES__ = undefined;
                        ...
                        "#, APP_ID, APP_ID, uuid::Uuid::new_v4().to_string(), APP_ID,
                    ).as_str())
                    .build()
