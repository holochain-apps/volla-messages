    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::Config(config) => {
                    validate_create_config(EntryCreationAction::Create(action), config)
                }
            ...
            OpEntry::UpdateEntry {
                ...
                EntryTypes::Config(config) => {
                    validate_create_config(EntryCreationAction::Update(action), config)
                }
                EntryTypes::Message(message) => {
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::ContactToContacts => {
                validate_create_link_contact_to_contacts(action, base_address, target_address, tag)
            }
            ...
            LinkTypes::RoomParticipants => todo!(),
            LinkTypes::ActiveCalls => todo!(),
        },
